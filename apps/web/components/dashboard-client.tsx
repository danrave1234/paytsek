  ...
  return <DashboardShell>
    ...
    {loading || !data ? <Loading /> : <>
      <section className="grid overflow-hidden rounded-[1.75rem] border border-line bg-bg lg:grid-cols-[1.35fr_.65fr]">
        <div className="p-7 sm:p-10"><p className="text-sm text-ink-3">Recorded today</p><p className="data mt-4 text-5xl font-semibold tracking-[-.06em] sm:text-6xl">{amount(data.home.today.recordedCentavos)}</p><p className="mt-3 text-sm text-ink-3">{data.home.today.recordedCount} record{data.home.today.recordedCount === 1 ? '' : 's'}</p></div>
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-[1.45fr_.85fr]">
        <Panel title="Latest records" action="Synced from mobile"><div className="divide-y divide-line">{data.home.recentRecords.length ? data.home.recentRecords.map((record) => <div key={record.id} className="grid grid-cols-[82px_1fr_auto] items-center gap-3 py-3.5"><p className="data text-xs text-ink-3">{recordTime(record, workspace?.timezone)}</p><div className="min-w-0"><p className="truncate text-sm font-semibold">{record.sourceLabel}</p><p className="mt-1 text-xs text-ink-3">{stateLabel(record.evidenceState)}</p></div><p className="data shrink-0 text-sm font-semibold">{amount(record.amountCentavos)}</p></div>) : <Empty label="No records yet today." />}</div></Panel>
        <Panel title="Payment phone"><div className="divide-y divide-line">{data.home.collectors.length ? data.home.collectors.map((collector) => <div key={collector.deviceId} className="flex items-center justify-between gap-3 py-3"><div><p className="text-sm font-semibold">{collector.sourceLabel}</p><p className="mt-1 text-xs text-ink-3">{collector.lastSeenAt ? `Seen ${time.format(new Date(collector.lastSeenAt))}` : 'Not connected yet'}</p></div><span className={`pill ${collector.stale ? 'bg-warn-soft text-warn' : 'bg-ok-soft text-ok'}`}>{collector.stale ? 'Needs attention' : 'Ready'}</span></div>) : <Empty label="Pair an Android payment phone to add notification evidence." />}</div></Panel>
      </section>
...
function SignIn({ message, onSignIn }: { message: string | null; onSignIn: () => void }) {
  ...
  return <div className="card-raised mx-auto max-w-lg p-7 sm:p-10">
    ...
    <button onClick={onSignIn} className="mt-7 inline-flex min-h-12 w-full items-center justify-center rounded-xl bg-ink px-5 text-sm font-semibold text-white hover:bg-brand">Continue with Google</button>
    ...
    {!emailOpen ? <button onClick={() => setEmailOpen(true)} className="inline-flex min-h-12 w-full items-center justify-center rounded-xl border border-line bg-bg px-5 text-sm font-semibold">Continue with email</button> : <form onSubmit={submit} className="space-y-3">
      ...
      <button type="submit" disabled={busy} className="min-h-12 w-full rounded-xl bg-brand px-5 text-sm font-semibold text-white disabled:opacity-60">{busy ? 'Please wait…' : mode === 'sign-in' ? 'Sign in' : 'Create account'}</button>
    </form>}
