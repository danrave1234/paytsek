...
PayTsek is a free public beta for Philippine sellers. A cashier scans the customer's receipt; the seller's own **Android** phone reports supported incoming-payment notifications; the server reconciles the two under a strict, versioned matching policy. Nothing here moves money, asks for wallet credentials, or claims provider verification.
...
apps/api                  NestJS API + Postgres-backed worker (reconciliation, exports, retention)
apps/web                  Next.js 16 + Tailwind v4 site: home, updates, downloads, privacy, terms, support
packages/contracts        Shared enums, Zod schemas, and error codes
...
supabase/migrations       Reviewed SQL: schema, constraints, RLS, and jobs
...
- Public beta records are never quota-metered or blocked; a stable client record id still prevents duplicates.
