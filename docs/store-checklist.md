...
- [ ] Reviewer test account: owner + cashier in a **demo workspace** (`organizations.is_demo = true`). Demo records never mix with production totals. Provide a scripted walkthrough: sign in → scan the provided sample receipt image → see Unverified → Review screen → Confirm manually.
...
- [ ] Sign-in with Google OAuth; provide reviewer credentials and ensure account deletion is available in Settings.
